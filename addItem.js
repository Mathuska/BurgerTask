...
const add = (loc, ig, deleteDiv) => {
  ...
  const deleteItem = (location) => {
    const itemToDelete = location.querySelectorAll(
      `[data-index="${btnClone.dataset.index}"]`
    );
...
addBottomSauceBtn.addEventListener("click", () =>
  add(divBottomSauce, bottomSauce, divDelBottom)
);
addTopSauceBtn.addEventListener("click", () =>
  add(divTopSauce, topSauce, divDelTop)
);
addCheeseBtn.addEventListener("click", () =>
  add(divCheese, cheese, divDelChese)
);
addToppingsBtn.addEventListener("click", () =>
  add(divToppings, toppings, divDelToppings)
);
